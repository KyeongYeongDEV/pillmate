...
## 🧭 작업 모델 (2026-09-02 단일세션 + 온디맨드 서브에이전트)

> 기존 cmux 6패널(BE/FE/QA/Reviewer/Adversarial/Ops 상시 세션)은 **폐기**. 하나의 메인 세션이 일하고, 필요할 때 아래 서브에이전트를 **Agent 툴로 호출**한다.

- **간단한 건 직접**: 메인 세션이 1~2줄 변경·설정·문서·하네스는 직접 편집한다 (코드 직접수정 차단 훅 제거됨).
- **복잡/전문/병렬은 위임**: 도메인 구현, 대량 조사, 멀티파일 변경, 성능 튜닝은 서브에이전트에 spec 을 주고 시킨다. 서로 다른 파일이면 병렬 OK, 같은 파일은 직렬(scope-discipline).
- **긴 조사·구현은 위임 우선**: 도구 출력이 메인 컨텍스트에 안 쌓여 컨텍스트 위생에 유리(context-hygiene).
- **Tier 1 QA(의료·보안·데이터·결제·배포·개인정보)**: 필요 시 Reviewer/QA-Claude/⚔️Adversarial 서브에이전트를 띄워 검증 후 커밋(qa-risk-tiers).
- **커밋/푸시**: 검증·게이트 통과 후 메인 세션이 수행.
...
> 메인 세션과 모든 서브에이전트는 작업 전 이 레이어를 먼저 읽는다. "잘", "깔끔히" 같은 형용사가 아니라 체크 가능한 기준.
...
| 2026-09-02 | 작업 모델 전환: cmux 6패널 폐기 → 단일 메인 세션 + 온디맨드 서브에이전트(Agent 툴). 코드 직접수정 차단 훅·.cmux/ 제거, 간단한 건 직접·복잡한 건 위임, Tier1 QA는 필요 시 서브에이전트 |
