-- dev/staging only — user_id=1 테스트 데이터 SEED. 운영 배포 전 별도 검토 필요.
-- 멱등 보장: WHERE NOT EXISTS 사용. 재실행 안전.

DO $$
DECLARE
    v_group_id BIGINT;
BEGIN
    -- 1. user_id=1 가 어느 그룹에도 없으면 테스트 그룹 + 멤버십 생성
    ...
    -- 3. 오늘 날짜 dose_logs PENDING 삽입 (없는 schedule 만)
