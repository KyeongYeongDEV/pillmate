...
public class GlobalExceptionHandler {
    ...
    /**
     * 도메인 검증 실패(IllegalArgumentException) — 클라이언트 입력 문제이므로 400, Sentry 전송 제외.
     */
    ...
    /**
     * 쿼리·경로 파라미터 타입 변환 실패 (예: month=2026-13, month=notamonth) — 클라이언트 문제이므로 Sentry 전송 제외.
     * 이게 없으면 catch-all(Exception) 로 떨어져 500 + Sentry 노이즈가 됨.
     */
    ...
    private HttpStatus resolveStatus(ErrorCode code) {
        return switch (code) {
            case DRUG_NOT_FOUND, GROUP_NOT_FOUND, PRESCRIPTION_NOT_FOUND, SCHEDULE_NOT_FOUND,
                 ...
                 NOTIFICATION_NOT_FOUND, INVALID_NOTIFICATION_DOSE_LOG, ACTIVITY_FEED_NOT_FOUND -> HttpStatus.NOT_FOUND;
            case GROUP_ACCESS_DENIED, NOT_NOTIFICATION_OWNER, PATIENT_ACCESS_DENIED,
                 ADMIN_ACCESS_DENIED, MEDICATION_SHARE_NOT_GRANTED -> HttpStatus.FORBIDDEN;
            case GROUP_INVITE_CODE_EXPIRED, INVITE_CODE_EXPIRED_OR_INVALID -> HttpStatus.GONE;
            case GROUP_INVITE_CODE_USED, GROUP_ALREADY_MEMBER, GROUP_MEMBER_LIMIT_EXCEEDED, SCHEDULE_CONFLICT,
                 SCHEDULE_PERIOD_ENDED, DOSE_LOG_DATE_LOCKED, DOSE_LOG_ALREADY_CHECKED,
                 NUDGE_NO_OVERDUE_DOSE -> HttpStatus.CONFLICT;
            case DRUG_SEARCH_EMPTY_QUERY, INVALID_REQUEST,
                 ...
                 OCR_REQUEST_INVALID, MEDICATION_SHARE_INVALID_TARGET, NUDGE_TARGET_INVALID,
                 PRAISE_TARGET_INVALID -> HttpStatus.BAD_REQUEST;
            ...
            case REPORT_REFRESH_RATE_LIMITED, RATE_LIMIT_EXCEEDED, NUDGE_COOLDOWN_ACTIVE -> HttpStatus.TOO_MANY_REQUESTS;
            ...
            case KAKAO_AUTH_FAILED, INVALID_AUTH_TOKEN, ACCOUNT_WITHDRAWN -> HttpStatus.UNAUTHORIZED;
