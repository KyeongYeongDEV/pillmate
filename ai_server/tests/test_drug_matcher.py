...
async def test_matcher_strips_brackets_before_ilike():
    ilike = StubIlikeSearch([MatchCandidate(kd_code="KD010", name="이세틸정100밀리그람", score=Decimal("1.0"))])
    vector = StubVectorSearch(None)
    matcher = DrugMatcher(ilike=ilike, vector=vector)
...
@pytest.mark.asyncio
async def test_matcher_normalizes_english_mg_to_korean():
    ilike = StubIlikeSearch(
        [MatchCandidate(kd_code="KD020", name="동광나자티딘캡슐150밀리그람", score=Decimal("1.0"))]
    )
    vector = StubVectorSearch(None)
    matcher = DrugMatcher(ilike=ilike, vector=vector)
    ...
    assert ilike.calls == ["동광나자티딘캡슐150밀리그람"]
...
async def test_matcher_falls_back_to_first_token_when_normalized_misses():
    ilike = StubIlikeSearch(
        [None, MatchCandidate(kd_code="KD030", name="동광나자티딘캡슐", score=Decimal("0.95"))]
    )
    vector = StubVectorSearch(None)
    matcher = DrugMatcher(ilike=ilike, vector=vector)
    ...
    assert ilike.calls == ["동광나자티딘캡슐150밀리그람", "동광나자티딘캡슐"]
...
@pytest.mark.asyncio
async def test_matcher_returns_none_when_all_three_stages_fail():
    ilike = StubIlikeSearch([None, None])
    vector = StubVectorSearch(None)
    matcher = DrugMatcher(ilike=ilike, vector=vector)

    result = await matcher.match(_raw("엔테론정150밀리그람 (포도씨건조엑스 150mg)", "0.90"))

    assert result.item.kd_code is None
    assert result.stage == "none"
    assert ilike.calls == ["엔테론정150밀리그람", "엔테론정"]
    assert vector.calls == ["엔테론정150밀리그람 (포도씨건조엑스 150mg)"]
