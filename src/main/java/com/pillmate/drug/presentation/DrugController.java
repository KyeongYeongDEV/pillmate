...
public class DrugController {
    ...
    @GetMapping("/search")
    public ResponseEntity<ApiResponse<List<DrugSearchResult>>> search(
            @RequestParam String q) {
        return ResponseEntity.ok(ApiResponse.success(searchDrugUseCase.search(q)));
    }

    @GetMapping("/{drugId}")
    public ResponseEntity<ApiResponse<DrugDetailResponse>> getDetail(
            @PathVariable Long drugId) {
        return ResponseEntity.ok(ApiResponse.success(getDrugDetailUseCase.getDetail(drugId)));
    }
}
